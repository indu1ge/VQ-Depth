...
from utils_vae.train_api import save_model
from utils_vae.logger import get_logger

from datasets.cityscapes import CityscapesColorDataset
from utils_vae.metrics import eval_depth, cropping_img
...
def validation(vae, val_data_loader, logger):
    ...
    with torch.no_grad():
        ...
        for iter, inputs in enumerate(val_data_loader):
            image = inputs['depth'].cuda()
    ...
    for epoch in range(EPOCHS):
        ...
        for iter, inputs in enumerate(train_data_loader):
            if cfg.train_setting.data.mask:
                mask_image = inputs['mask_depth'].cuda()
                image = inputs['depth'].cuda()
...
def main(cfg, local_rank=0):
    ...
    train_dataset = CityscapesColorDataset(**train_dataset_kwargs)
    ...
    val_dataset = CityscapesColorDataset(**val_dataset_kwargs)
